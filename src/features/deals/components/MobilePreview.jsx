...
const MobilePreview = ({ formData }) => {
  ...
  const retail = formData.retail || '$10.99';
  const coupon = formData.coupon || '0.00';
  const reward = formData.reward || '0.00';
  const expiry = formData.endDate || '24/06/26';
  
  // Calculate final price logic for the preview
  // Just strip $ and parse to float for a dummy calculation
  const retailNum = parseFloat(retail.replace(/[^0-9.]/g, '')) || 10.99;
  const couponNum = parseFloat(coupon.replace(/[^0-9.]/g, '')) || 3.00;
  const finalPrice = (retailNum - couponNum).toFixed(2);
            ...
            <div className="flex-1 p-2 flex flex-col gap-1.5">
                ...
                <div className="flex flex-col justify-between flex-1">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-col items-end leading-none">
                      ...
                      <span className="text-[9px] font-bold text-[#0A0A0A] line-through decoration-1">{retail}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="bg-[#E8F0FE] text-[#005EF8] rounded-sm flex items-center px-1.5 py-0.5 gap-1">
                  ...
                  <span className="text-[9px] font-bold">${parseFloat(reward.replace(/[^0-9.]/g, '') || 0).toFixed(2)}</span>
                </div>
              </div>
